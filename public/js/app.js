...
function layout(user){
  ...
  <aside class="sidebar"><div class="sidebar-brand"><div class="sidebar-logo"><img src="assets/images/isotipo-activa-white.png" alt="Activa Tu Corazón"></div><div><strong>Activa 360</strong><small>Plataforma pastoral · v1.2 Cloud</small></div></div><nav class="nav">${allowed.map(r=>`<button class="nav-btn" data-route="${r}"><span class="nav-icon">${routes[r].icon}</span><span class="nav-label">${routes[r].label}</span></button>`).join('')}</nav><div class="sidebar-footer"><div class="mini-brand">Activa Tu Corazón · Bogotá Norte</div></div></aside>
