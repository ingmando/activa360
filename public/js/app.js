...
function allowedRoutes(role){
  ...
  if(role==='pastor'||role==='superadmin')return all;
...
function layout(user){
  ...
  <aside class="sidebar" id="sidebar"><div class="sidebar-brand"><div class="sidebar-logo"><img src="assets/images/isotipo-activa-white.png" alt="Activa Tu Corazón"></div><div><strong>Activa 360</strong><small>Plataforma pastoral · v1.3.5</small></div><button class="sidebar-close" id="sidebarClose" aria-label="Cerrar menú">×</button></div><nav class="nav">${sideNav}</nav><div class="sidebar-footer"><div class="mini-brand">Activa Tu Corazón · Bogotá Norte</div></div></aside><div class="sidebar-scrim" id="sidebarScrim"></div>
...
async function renderApp(){
  ...
  document.querySelector('#menuTrigger')?.addEventListener('click',()=>{
    if(window.matchMedia('(max-width: 767px)').matches){ closeNav(); openMoreMenu(user); }
    else openNav();
  });
  document.querySelector('#sidebarClose')?.addEventListener('click',closeNav); document.querySelector('#sidebarScrim')?.addEventListener('click',closeNav);
  document.querySelector('#moreNavBtn')?.addEventListener('click',()=>{closeNav();openMoreMenu(user);});
...
window.addEventListener('hashchange',()=>{document.querySelector('#userDropdown')?.setAttribute('hidden','');document.body.classList.remove('nav-open');const u=getSession();if(u)renderContent(u);});
