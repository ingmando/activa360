...
function explorerMarkup(title,subtitle,items,people,parentId='',trail=[]){
  const crumbs=trail.length?`<div class="tree-breadcrumb" aria-label="Ruta ministerial">${trail.map((t,i)=>`<span>${i?'›':''}${esc(t)}</span>`).join('')}</div>`:'';
  return `${crumbs}<div class="tree-explorer-head"><div><span class="eyebrow">Explorador ministerial</span><h3>${esc(title)}</h3><p>${esc(subtitle)}</p></div>${parentId?'<button class="btn btn-soft btn-sm" id="treeBack">← Volver</button>':''}</div><div class="tree-explorer-grid">${items.map(p=>node(p,people)).join('')||'<div class="empty-state">No hay personas registradas en este nivel.</div>'}</div>`
}
export async function renderMinisterialTree(){
  ...
  <section class="card section-card tree-explorer" id="treeExplorer">${explorerMarkup(label(0),'Cobertura pastoral general',roots,people,'',[label(0)])}</section>`;
}
export async function bindMinisterialTree(){
  ...
  const bindNodes=()=>document.querySelectorAll('[data-tree-person]').forEach(b=>b.onclick=()=>{const id=Number(b.dataset.treePerson),p=people.find(x=>Number(x.id)===id),kids=childrenOf(people,id);if(kids.length){history.push({id,title:p.name,level:Number(p.leadershipLevel||0)});explorer.innerHTML=explorerMarkup(p.name,`${label(Number(p.leadershipLevel||0)+1)} · ${kids.length} integrante${kids.length===1?'':'s'}`,kids,people,id,[label(0),...history.map(h=>h.title)]);bindNodes();bindBack();}else location.hash=`#/personas/${id}`;});
  const bindBack=()=>{document.querySelector('#treeBack')?.addEventListener('click',()=>{history.pop();const prev=history[history.length-1];if(prev){const p=people.find(x=>Number(x.id)===Number(prev.id)),kids=childrenOf(people,p.id);explorer.innerHTML=explorerMarkup(p.name,`${label(Number(p.leadershipLevel||0)+1)} · ${kids.length} integrantes`,kids,people,p.id,[label(0),...history.map(h=>h.title)]);}else{const roots=levelPeople(people,0);explorer.innerHTML=explorerMarkup(label(0),'Cobertura pastoral general',roots,people,'',[label(0)]);}bindNodes();bindBack();});};
  document.querySelectorAll('[data-tree-level]').forEach(btn=>btn.onclick=()=>{document.querySelectorAll('[data-tree-level]').forEach(x=>x.classList.remove('active'));btn.classList.add('active');history.length=0;const level=Number(btn.dataset.treeLevel),items=levelPeople(people,level);explorer.innerHTML=explorerMarkup(label(level),`${items.length} personas en esta generación`,items,people,'',[label(level)]);bindNodes();});
  bindNodes(); bindBack();
}
