...
export async function renderPeopleList(user) {
  ...
  const canCreate = ['pastor','leader12','cellLeader'].includes(user.role);
...
export async function renderPersonDetail(id,user) {
  ...
  const cell=cells.find(c=>c.id===p.cellId); const personFollowups=followups.filter(x=>x.personId===p.id); const tr=training.find(x=>x.personId===p.id); const personMinistries=ministryMembers.filter(x=>x.personId===p.id&&x.status!=='Inactivo').map(x=>({membership:x,ministry:ministries.find(m=>m.id===x.ministryId)})).filter(x=>x.ministry); const canEdit=['pastor','leader12','cellLeader'].includes(user.role); const canDelete=['pastor','leader12'].includes(user.role);
