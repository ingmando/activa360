...
function metricInput(name,label,value,autoValue){const manual=value!==undefined&&value!==null;return '<div class="metric"><label for="'+name+'">'+esc(label)+'</label><input id="'+name+'" name="'+name+'" type="number" min="0" step="1" value="'+esc(manual?value:autoValue)+'" required><small>'+(manual?'Valor guardado manualmente':'Valor sugerido por Activa 360')+'</small></div>'}
function render(d){const members=d.members||[],dev=d.report?.devotionals||{},manual=d.report?.manualMetrics||{};app.innerHTML='<span class="status">'+esc(d.meeting.status)+'</span><h1>'+esc(d.unit.displayName)+'</h1><p class="muted">'+esc(d.meeting.title)+' · '+esc(d.meeting.date)+' · '+esc(d.meeting.time||'18:30')+'</p><h2>Reporte semanal</h2><p class="muted">El devocional se registra por persona. Diezmo, ofrenda y altar familiar corresponden al equipo ministerial.</p><form id="f">'+members.map(m=>yn('dev_'+m.id,dev[String(m.id)],'Devocional · '+m.firstName)).join('')+'<div class="grid">'+yn('tithe',d.report?.tithe,'Diezmo')+yn('offering',d.report?.offering,'Ofrenda')+yn('familyAltar',d.report?.familyAltar,'Altar familiar')+'</div><h2 style="margin-top:22px">Información ministerial</h2><p class="muted">Mientras consolidamos los módulos reales, puedes corregir estos valores. El dato automático queda como referencia.</p><div class="grid">'+metricInput('metric_cells','Células',manual.cells,d.auto.cells)+metricInput('metric_meetingsDone','Realizadas',manual.meetingsDone,d.auto.meetingsDone)+metricInput('metric_schoolPeople','En escuela',manual.schoolPeople,d.auto.schoolPeople)+metricInput('metric_encounterPeople','Encuentro',manual.encounterPeople,d.auto.encounterPeople)+'</div><div class="field"><label>Observaciones / compromisos</label><textarea name="notes" rows="4">'+esc(d.report?.notes||'')+'</textarea></div><button class="btn">Guardar reporte</button></form>';document.querySelector('#f').onsubmit=submit;}
async function submit(e){e.preventDefault();const f=new FormData(e.target),num=k=>Math.max(0,Number(f.get(k)||0)),payload={devotionals:{},tithe:f.get('tithe')==='true',offering:f.get('offering')==='true',familyAltar:f.get('familyAltar')==='true',manualMetrics:{cells:num('metric_cells'),meetingsDone:num('metric_meetingsDone'),schoolPeople:num('metric_schoolPeople'),encounterPeople:num('metric_encounterPeople')},notes:String(f.get('notes')||'')};for(const [k,v] of f.entries())if(k.startsWith('dev_'))payload.devotionals[k.slice(4)]=v==='true';const b=e.target.querySelector('button');b.disabled=true;b.textContent='Guardando...';try{const r=await fetch('/api/public/r12/'+encodeURIComponent(token),{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(payload)});const d=await r.json();if(!r.ok)throw new Error(d.error||'No fue posible guardar');app.innerHTML='<div class="done"><strong>Reporte guardado correctamente.</strong><p>Puedes volver a abrir este mismo enlace mientras la reunión permanezca abierta.</p></div>';}catch(err){b.disabled=false;b.textContent='Guardar reporte';alert(err.message)}}load();
</script></body></html>`;
}
...
async function handleR12Public(request, env, token) {
  ...
  if(request.method==='POST') {
    ...
    const manualMetrics=body?.manualMetrics||{}; const normalizedMetrics={cells:Math.max(0,Number(manualMetrics.cells||0)),meetingsDone:Math.max(0,Number(manualMetrics.meetingsDone||0)),schoolPeople:Math.max(0,Number(manualMetrics.schoolPeople||0)),encounterPeople:Math.max(0,Number(manualMetrics.encounterPeople||0))}; const id=report?.id||await nextStoreId(env,'twelveMeetingReports'); const payload={...(report||{}),id,meetingId:Number(meeting.id),unitId:Number(unit.id),leaderId:Number(unit.primaryPersonId),devotionals,tithe:body.tithe,offering:body.offering,familyAltar:body.familyAltar,manualMetrics:normalizedMetrics,metricSource:'manual',notes:String(body.notes||'').trim(),submittedVia:'publicLink',updatedAt:new Date().toISOString()}; await saveRecord(env,'twelveMeetingReports',payload); invite.usedAt=new Date().toISOString();invite.updatedAt=invite.usedAt;await saveRecord(env,'twelveMeetingInvites',invite);return json({ok:true,report:payload});
  }
  return json({error:'Operación no soportada'},405);
}
